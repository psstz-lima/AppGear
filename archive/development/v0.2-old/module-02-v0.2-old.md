...
- Confirma a borda única Traefik → Coraza → Kong → Istio com mTLS STRICT e validação OPA/Kyverno.
...
Define Traefik (ou outro ingress), WAF (Coraza), API Gateway (Kong) e Istio com mTLS para proteger rotas e serviços.
...
1. A **malha de serviço interna** com **Istio** e **mTLS em modo STRICT** para todo tráfego Leste–Oeste entre serviços `core-*` e `ws-*`.
...
1. **Cumprir Contrato v0, Auditoria v0 e Interoperabilidade v0**
   ...
   * G06 – mTLS STRICT na malha de serviço (Istio);
...
* Documentar triggers e tempos em `guides/keda-scale-to-zero.md` e refletir nos charts Helm (Traefik/Coraza/Istio) para que o comportamento seja o padrão da topologia B.
...
### 1. Istio – Malha de Serviço com mTLS STRICT (apps/core/istio)
...
git commit -m "mod02: Istio com mTLS STRICT e labels de governanca (tenant global)"
...
### 1. Istio – mTLS STRICT e componentes
...
* **Alterar `mTLS` para `PERMISSIVE` em `PeerAuthentication`**
...
* **Referências cruzadas:**
  ...
  * `2 - Auditoria v0` deve usar a cadeia Traefik → Coraza → Kong → Istio e mTLS STRICT como critérios de conformidade;
...
* **Módulo 02 – Malha de Serviço e Borda (este módulo)**
  ...
  * Entrega:
    ...
    * Istio com mTLS STRICT na Topologia B,
...
* **Módulo 04 – Armazenamento e Bancos Core**
  ...
  * **Depende indiretamente deste módulo**:
    ...
    * todo tráfego de aplicações que consomem bancos/brokers passa pela malha (Istio mTLS STRICT),
