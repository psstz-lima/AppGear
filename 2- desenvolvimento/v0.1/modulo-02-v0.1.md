...
1.  **NC-001 (Estrutura de Seções):** A versão anterior alterou a numeração dos tópicos técnicos (4.1, 4.2...), dificultando a correlação com o v0 original.
      * *Status:* **Corrigido**. Retomada a numeração original (1 a 6) dentro de "Como fazer".
2.  **NC-002 (Nomenclatura):** Prefixos `webapp-ia-` e diretório `/opt/webapp-ia`.
      * *Status:* **Corrigido**. Padronizado para `appgear-` e `/opt/appgear`.
3.  **NC-003 (Segurança/Imagens):** Exemplos com `:latest` e `.env` vulnerável.
      * *Status:* **Corrigido**. Versões pinadas e `.gitignore` padrão incluídos.
4.  **NC-004 (FinOps):** Ausência de labels de custo nos exemplos.
      * *Status:* **Corrigido**. `appgear.io/tenant-id` adicionado aos manifestos.
5.  **NC-005 (Terminologia):** Uso de "Retrofit".
      * *Status:* **Corrigido**. Substituído por "Revisado".

### PLANO\_CORRECAO

1.  **Fidelidade Estrutural:** Manter os tópicos: 1. Forma canônica, 2. Estrutura doc, 3. Topologia A, 4. Topologia B, 5. Policy as Code, 6. Segredos.
2.  **Refatoração de Código:** Atualizar todos os scripts Bash, YAMLs e Rego para usar `appgear-net-*`, `/opt/appgear` e repositórios `appgear-*`.
3.  **Enriquecimento:** Garantir que os exemplos de `k8s` tenham `resources` e `probes` preenchidos conforme a regra.
...
> **Status:** Fonte da Verdade para Nomenclatura e Padrões Técnicos.
> **Alterações:** Migração de redes, aviso explícito de Topologia A, resources e segurança em Dev.
...
#### 3.1 Diretório raiz no host (Padrão `/opt/appgear`)
...
* [x] **Fidelidade Estrutural:** Seções 1, 2, 3, 4(1-6), 5, 6, 7 reproduzidas exatas do v0.
* [x] **Conteúdo Técnico:** Healthchecks, OPA, Scripts de Migração e CI Workflow preservados.
* [x] **Correção Nomenclatura:** `appgear-`, `/opt/appgear` aplicados.
* [x] **FinOps:** Injeção de `tenant-id` nos exemplos de K8s e OPA.
* [x] **Terminologia:** "Revisado" aplicado.
* [x] **Imagens:** `:latest` removido dos exemplos.
```