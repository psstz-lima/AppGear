...
1.  **NC-001 (Nomenclatura de Repositórios):** O módulo original usava prefixos mistos ou indefinidos.
      * *Correção:* Padronização para `appgear-*` (ex: `appgear-gitops-core`).
2.  **NC-002 (Estrutura Local):** O diretório de trabalho não estava padronizado.
      * *Correção:* Definido `/opt/appgear` como raiz obrigatória para Topologia A.
3.  **NC-003 (Redes Docker):** Nomes de rede legados (`core-net`) causavam conflito.
      * *Correção:* Redes padronizadas para `appgear-net-core` e `appgear-net-apps`.
4.  **NC-004 (Terminologia):** Uso do termo "Retrofit".
      * *Correção:* Alterado para "Revisado".
5.  **NC-005 (Segurança):** Uso de imagens `:latest`.
      * *Correção:* Substituído por versões pinadas nos exemplos.

### PLANO\_CORRECAO

1.  **Preservação Total:** Manter todas as seções técnicas (Healthchecks, OPA, Scripts de Migração) do arquivo v0 original.
2.  **Search & Replace Inteligente:**
      * `webapp-ia-` → `appgear-` (em repositórios).
      * `/opt/webapp-ia` → `/opt/appgear`.
      * `core-net` → `appgear-net-core` (nos exemplos de configuração nova).
3.  **Ajuste de Versão:** Atualizar cabeçalho para v0.1 (Revisado).
...
> **Versão:** v0.1 (Revisado)
> **Status:** Fonte da Verdade para Governança, Nomenclatura e Padrões Técnicos.
> **Alterações:** Migração de redes, aviso explícito de Topologia A, recomendação de resources (CPU/RAM) e reforço de segredos em Dev.
...
* **Forma canônica:** Todo artefato oficial deve ser **Markdown (`.md`)**.
...
Ele é a **base**: qualquer módulo técnico (01..17) que não respeitar este módulo é considerado **não conforme**.
...
> **AVISO IMPORTANTE:** Topologia A é **apenas para DEV, testes locais ou PoC**. Não é recomendada para produção.
...
Se já existir ambiente rodando com redes antigas:

1.  Planeje **janela de manutenção**.
...
#### 4.4.1 Nomes de clusters, namespaces, tenants e workspaces
  ...
  * Cluster físico: `ag-<regiao>-core-<env>` (Ex.: `ag-br-core-dev`).
  * Namespaces globais: `argocd`, `appgear-core`, `observability`, `security`, `backstage`.
  * Tenants: `tenant-<slug>`.
  * Workspaces: `kebab-case`.
  * vCluster: `vcl-ws-<workspace_id>`.
  * Namespaces por workspace: `ws-<workspace_id>-core`, `ws-<workspace_id>-factory`.
...
* [x] **Conteúdo Completo:** Todos os comandos, healthchecks e policies da v0 original estão presentes.
* [x] **Correção de Nomenclatura:** Repositórios `appgear-*`, diretórios `/opt/appgear` e redes `appgear-net-*`.
* [x] **Versão:** Cabeçalho e referências atualizadas para `v0.1`.
* [x] **Terminologia:** Uso de "Revisado" (não "Retrofit" ou "Runbook").
* [x] **Estrutura:** Seções 1-7 preservadas integralmente.
```