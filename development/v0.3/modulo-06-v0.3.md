# M06 – Identidade e SSO (Keycloak, midPoint, RBAC/ReBAC) (v0.3)

> [!IMPORTANT]
> Este documento define o **Módulo 06 (M06)** da arquitetura AppGear na linha v0.3.  
...
- Cadeia obrigatória `Traefik (TLS passthrough SNI) → Coraza WAF → Kong → Istio IngressGateway → Service Mesh` com mTLS **STRICT** e exceções documentadas no quadro de monitoramento.
...
* **Governança:** registrar os parâmetros (trigger, `pollingInterval`, `cooldownPeriod`) na documentação do módulo e no repositório `4-documentos/` (ver `4-documentos/keda-scale-to-zero.md`).
...
* **Módulo 02 – Malha de Serviço e Borda (Istio, Traefik, Coraza, Kong)**
  ...
  * Fornece:
    ...
    * malha de serviço com `mTLS STRICT STRICT`, protegendo o tráfego para Keycloak/midPoint/OpenFGA,
