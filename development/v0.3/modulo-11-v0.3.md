# M11 – Suíte Operations (IoT, Digital Twins, RPA, KubeEdge) (v0.3)

> [!IMPORTANT]
> Este documento define o **Módulo 11 (M11)** da arquitetura AppGear na linha v0.3.  
...
1. **Endereçar não conformidades do diagnóstico (G15, G05, M00-3)** 
   ...
   * **G05 – Segurança de Borda IoT**
     ...
     * Entrada de **MQTT/CoAP** e **CloudCore** explicitada como:
       ...
       * Protegida por mTLS STRICT, firewall e IP/VIP controlado.
...
2. **Cadeia de rede coerente com o Módulo 2**
   ...
   * Protocolos não HTTP (MQTT, CoAP, CloudCore TCP):
     ...
     * Entram via borda **TCP/UDP dedicada**, documentada como exceção:
       ...
       * Segurança baseada em mTLS STRICT, firewall, limitação de origem.
...
#### 2.4 MQTT – borda TCP dedicada (sem WAF HTTP, com mTLS STRICT)
...
* Segurança vem de:
  ...
  * **mTLS STRICT** (client cert nos devices);
...
> O provedor de cloud atribui um IP externo (VIP) a esse LB, registrado no DNS e liberado no firewall. Segurança via DTLS/mTLS STRICT + firewall.
...
* Correção:
  ...
  * MQTT/CoAP usam borda TCP/UDP dedicada, protegida por mTLS STRICT e firewall.
