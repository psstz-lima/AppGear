# M01 – GitOps e Argo CD (App-of-Apps) (v0.3)

> [!IMPORTANT]
> Este documento define o **Módulo 01 (M01)** da arquitetura AppGear na linha v0.3.  
...
4. **Estrutura GitOps App-of-Apps**
 ...
 5. **Migração App-of-Apps → ApplicationSet (list-generator)**

   * **Papel do App-of-Apps:** fica restrito a instalar o Argo CD e registrar apenas os `Applications` raiz (`root-core-*` e `root-suites-*`). Nenhum `Application` isolado deve representar workspaces.
   * **ApplicationSet Core/Suites (list-generator):** em `appgear-gitops-core` e `appgear-gitops-suites`, criar `ApplicationSet` com `generators.list.elements` para cada cluster/ambiente Core.
   * **ApplicationSet Workspaces/vClusters:** em `appgear-gitops-workspaces`, criar um `ApplicationSet` único que gere 1 `Application` por combinação `workspace_id`/`vcluster_id`, permitindo **incluir/remover workspaces via PR**. Exemplo base:
       ...
       - list:
           elements:
             - name: ws-alpha
               tenant: acme
               env: dev
               vcluster: vcluster-alpha
             - name: ws-beta
               tenant: acme
               env: prod
               vcluster: vcluster-beta
     template:
       metadata:
         name: "ws-{{name}}"
         labels:
           appgear.io/tenant: "{{tenant}}"
           appgear.io/workspace: "{{name}}"
   ...
   * **Benefício:** governança centralizada e dinâmica de workspaces/vClusters, sem drift entre ambientes e sem necessidade de editar o App-of-Apps a cada inclusão.

 6. **Padronização de layout em `/opt/appgear` e `.env` central**
